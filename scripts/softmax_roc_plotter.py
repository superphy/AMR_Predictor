...
if __name__ == "__main__":
	...
	for drug in df_cols:
		...
		matrix = np.load("softmax_matrices/"+drug+".npy")
	...
	st = f.suptitle("Softmax - acc", fontsize=20)
	st = g.suptitle("Softmax - npreds", fontsize=20)
	plt.show()
...
#		#plt.plot(z)
#		#plt.title(drug+' sigmoid')#if __name__ == "__main__":
#
#   df = joblib.load(os.path.abspath(os.path.curdir)+"/amr_data/mic_class_dataframe.pkl")
#   df_rows = df.index.values   # Row names are genomes
#
