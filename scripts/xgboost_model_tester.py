...
if __name__ == "__main__":
	...
	for drug in df_cols:
		...
		for fold in range(1,6):
			...
			#sk_obj = SelectKBest(f_classif, k=270)
			...
			loaded_model  = pickle.load(open(os.path.abspath(os.path.curdir)+'/filtered_data/amr_data/'+drug+'/270feats/fold'+str(fold)+'/xgb_model.dat', "rb"))
			print(loaded_model.best_model()["learner"])
			model = loaded_model.best_model()['learner']

			model.fit(x_train, y_train)
			#score, acc = model.evaluate(x_test, y_test, verbose=0)

			#saving model, can be loaded with     model = load_model('my_model.hdf5')
			#fname = "NN_Models/NN_Model" + drug + str(split_counter)+'.hdf5'
			#model.save(fname)
			...
			OBOResults = eval_modelOBO(model, x_test, y_test)
			...
			report = classification_report(results[3], results[2],labels=labels)
			report_scores.append(report)
			cvscores.append(results[0])
			print('Base Acc:',results[0])
			print('1-D Acc: ',OBOResults[0])
			print('MCC:     ',results[1])
			print(report)
			#find_errors(model, x_test, y_test, grdi_rows_gen, mic_class_dict[drug], drug, mic_class_dict)
