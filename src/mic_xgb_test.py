...
if __name__ == "__main__":
	...
	for drug in df_cols:
			...
			#create dataframe count of how many genomes appear in that mic value
			print("before Y load")
			...
			dict(c)
			print(c)
			df_mic = pd.DataFrame.from_dict(c, orient='index').reset_index()
			df_mic = df_mic.rename(columns={'index':'MIC', 0:'Count'})
			print(df_mic)

			#plot the results of the DataFrame
			sns.set(style="whitegrid")
			ax = sns.barplot(x="MIC", y="Count", data=df_mic)
			break
