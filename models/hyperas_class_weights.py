...
def find_errors(model, test_data, test_names, genome_names, class_dict, drug, mic_class_dict):
	...
	err_file = open(os.path.abspath(os.path.curdir)+'/amr_data/errors/'+str(sys.argv[1])+'_feats_hyperas_weights_errors.txt', 'a+')
...
if __name__ == "__main__":
	##################################################################
	# call with
	#	time python hyperas_class_weights.py <numfeats> <drug> <fold>
	# to do all folds
	#	for i in {1..5}; do python hyperas_class_weights.py <numfeats> <drug> '$i'; done
	# to do all folds on waffles
	#	sbatch -c 16 --mem 80G --partition NMLResearch --wrap='for i in {1..5}; do python hyperas_class_weights.py <numfeats> <drug> "$i"; done'
	# OR
	#   or use hyperas_class_weights.snake (change the features num)
	##################################################################
	feats = sys.argv[1]
	drug  = sys.argv[2]
	fold  = sys.argv[3]
	...
	print("************************************")
	print("hyperas_class_weights.py")
	...
	rep_df = metrics_report_to_df(y_true, y_pred, mic_class_dict[drug])
	################################################################

	## Save Everything #############################################
	#best_model.save(filepath+'weight_model.hdf5')
	conf_df.to_pickle(filepath+'weight_conf_df.pkl')
	score_df.to_pickle(filepath+'weight_score_df.pkl')
	rep_df.to_pickle(filepath+'weight_rep_df.pkl')
