...
def find_errors(model, test_data, test_names, genome_names, class_dict, drug, mic_class_dict):
	...
	err_file = open(os.path.abspath(os.path.curdir)+'/roary_amr/errors/'+str(sys.argv[1])+'_feats_svm_errors.txt', 'a+')
...
if __name__ == "__main__":
	##################################################################
	# call with
	#	time python roary_svm.py <numfeats> <drug> <fold>
	# to do all folds
	#	for i in {1..5}; do python roary_svm.py <numfeats> <drug> '$i'; done
	# to do all folds on waffles
	#	sbatch -c 16 --mem 80G --partition NMLResearch --wrap='for i in {1..5}; do python roary_svm.py <numfeats> <drug> "$i"; done'
	# OR
	#   or use roary_svm.snake (change the features num)
	##################################################################
