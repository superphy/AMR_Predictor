...
if __name__ == "__main__":
	...
	for drug in df_cols:
		...
		grdi_matrix = np.load('non_grdi/amr_data/'+drug+'/kmer_matrix.npy')
		#print(matrix.shape)
		grdi_rows_mic = np.load('non_grdi/amr_data/'+drug+'/kmer_rows_mic.npy')
		grdi_rows_gen = np.load('non_grdi/amr_data/'+drug+'/kmer_rows_genomes.npy')
		...
		if(1==1):
			...
			fname = os.path.abspath(os.path.curdir)+"/all_feats_models/"+'ann_orig_as_test_'+ drug + str(split_counter)+'.hdf5'
			model.save(fname)
