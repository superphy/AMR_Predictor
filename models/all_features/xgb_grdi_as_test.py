...
if __name__ == "__main__":
	...
	for drug in df_cols:
		...
		for i in range(1):
			...
			model = XGBClassifier(learning_rate=1, n_estimators=10, objective='multi:softmax', silent=True, nthread=63)
			model.fit(x_train, y_train)

			#saving model, can be loaded with     model = load_model('my_model.hdf5')
			fname = os.path.abspath(os.path.curdir)+"/all_feats_models/"+'xgb_grdi_as_test_' + drug + str(split_counter)+'.dat'
			#model.save(fname)
