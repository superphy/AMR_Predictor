...
if __name__ == "__main__":
	...
	for drug in df_cols:
		...
		for train,test in cv.split(X,Y, Z):
			...
			#print("fitting")
			model.fit(x_train, y_train, epochs=1000, verbose=0, callbacks=[early_stop, reduce_LR])
			#score, acc = model.evaluate(x_test, y_test, verbose=0)

			#saving model, can be loaded with     model = load_model('my_model.hdf5')
			fname = os.path.abspath(os.path.curdir)+"/all_feats_models/"+'ann_mix_data_' + drug + str(split_counter)+'.hdf5'
			fpath = os.path.abspath(os.path.curdir)+"/all_feats_models/"
